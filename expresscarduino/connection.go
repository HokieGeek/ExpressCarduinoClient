...
func (c *Connection) setBaudRate(rate uint32) (error) {
    descriptor := c.file.Fd()
    vtime := convertTimeToPosix(connectionTimeout)
    
    term := syscall.Termios{
        Iflag: syscall.IGNPAR,
        Cflag: syscall.C8 | syscall.CREAD | syscall.CLOCAL | rate,
...
func (c *Connection) Connect() (error) {
    // Open the file
    var err error
    c.file,err = os.OpenFile(c.serial.deviceName, syscall.O_RDWR | syscall.NO_CTTY | syscall.O_NONBLOCK, 0666)
    if err != nil {
        return err
    }
